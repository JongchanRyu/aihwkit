...
def save_results(model, train_losses, valid_losses, test_error, tag=0):
    """
    ...
    with open(os.path.join(PATH_RESULT, str(tag)+"_Model_info.txt"), "w") as f:
        f.write('Non-default settings: \n\n')
        ...
        for i in range(len(model)):
            try:
                f.write(f'\n\n{model[i].analog_tile.tile}')
            except AttributeError:
                pass
        

    np.savetxt(os.path.join(PATH_RESULT, str(tag)+"_Test_error.csv"), test_error, delimiter=",")
    np.savetxt(os.path.join(PATH_RESULT, str(tag)+"_Train_Losses.csv"), train_losses, delimiter=",")
    np.savetxt(os.path.join(PATH_RESULT, str(tag)+"_Valid_Losses.csv"), valid_losses, delimiter=",")


def plot_results(train_losses, valid_losses, test_error, tag=0):
    """Plot results.
    ...
    plt.savefig(os.path.join(PATH_RESULT, str(tag)+'_test_losses.png'))
    plt.close()
    ...
    plt.ylim((5e-1, 1e2))
    plt.grid(which='both', linestyle='--')
    plt.savefig(os.path.join(PATH_RESULT, str(tag)+'_test_error.png'))
    plt.close()
